    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(
                f"{device_service_url}/users/{user_name}/devices"
            )
            response.raise_for_status()
            home_state = response.json()

            # First time user — initialize devices then re-fetch
            if not home_state:
                init_response = await client.post(
                    f"{device_service_url}/users/{user_name}/devices/initialize"
                )
...
async def initialize_session_state(config):
    ...
    if "user_id" not in st.session_state:
        st.session_state.user_id = config["credentials"]["usernames"][
    ...
    if "home_state" not in st.session_state:
        ...
        with st.spinner("Loading home state..."):
            ...
            st.session_state["home_state_last_refresh"] = (
